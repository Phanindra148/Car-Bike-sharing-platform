  ...
  function renderVehicles(arr) {
    ...
    arr.forEach(v => {
      ...
      card.innerHTML = `
        ...
        <button class="book-button" onclick="bookVehicle(${v.id},${v.hours})">Book Now</button>
      `;
      list.appendChild(card);

      // Countdown: use stored expiry_iso
      const expiryDate = new Date(v.expiry_iso);
      startCountdown(`timer-${v.id}`, expiryDate, v.id);
    });
  }

  window.bookVehicle = (id, hrs) => {
    location.href = `/Car-Bike-Sharing-Platform/backend/payment_page.php?vehicleId=${id}&hours=${hrs}`;
  };
